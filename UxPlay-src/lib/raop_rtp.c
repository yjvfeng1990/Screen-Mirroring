...
raop_rtp_init_sockets(raop_rtp_t *raop_rtp, int use_ipv6)
{
    ...
    /* Enlarge the UDP receive buffers: on Windows the default is only 8 kB,
     * which overflows under audio burst traffic (mirroring video+audio),
     * causing heavy packet loss -> RTCP resend storms -> client disconnect. */
#ifdef _WIN32
    {
